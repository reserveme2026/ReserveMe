...
<body class="bg-violet-50 min-h-screen">
    ...
    <div class="max-w-2xl mx-auto px-4 py-10 flex flex-col gap-6">
        ...
        <h1 class="text-2xl font-bold text-violet-950">Crear bloqueo para {{ $employee->name }}</h1>
            ...
            <div class="p-6">
                ...
                <form action="{{ route('employees.blockedTimes.store', $employee) }}" method="post" class="flex flex-col gap-5">
                    ...
                    <div class="flex flex-col gap-1">
                        <label for="block_date" class="text-sm font-medium text-violet-700">Fecha</label>
                        <input type="date" name="block_date" id="block_date" value="{{ old('block_date') }}"
                            class="border @error('block_date') border-red-300 @else border-violet-200 @enderror rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-violet-400 transition">
                        @error('block_date')
                    ...
                    <div class="flex flex-col gap-1">
                        <label for="reason" class="text-sm font-medium text-violet-700">Motivo</label>
                        <input type="text" name="reason" id="reason" value="{{ old('reason') }}"
                            class="border @error('reason') border-red-300 @else border-violet-200 @enderror rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-violet-400 transition">
                        @error('reason')
                            <small class="text-red-500 text-xs">{{ $message }}</small>
                        @enderror
                    </div>
