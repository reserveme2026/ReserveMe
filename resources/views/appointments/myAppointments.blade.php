...
<body class="bg-violet-50 dark:bg-gray-900 min-h-screen transition-colors duration-200">
    ...
    <div class="max-w-7xl mx-auto px-4 py-10 flex flex-col gap-6">
        ...
        @if (session('success'))
            <div
                class="flex items-center gap-2 bg-emerald-50 dark:bg-emerald-950 border border-emerald-200 dark:border-emerald-800 text-emerald-700 dark:text-emerald-400 text-sm px-4 py-3 rounded-xl">
        ...
        @if (session('error'))
            <div
                class="flex items-center gap-2 bg-red-50 dark:bg-red-950 border border-red-200 dark:border-red-800 text-red-700 dark:text-red-400 text-sm px-4 py-3 rounded-xl">
                <svg class="w-4 h-4 shrink-0" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24">
        ...
        @if ($appointments->count() > 0)
                ...
                <table class="w-full text-sm text-left">
                    <thead>
                        <tr class="border-b border-violet-100 dark:border-gray-700">
                            <th class="px-4 py-3 text-[11px] font-bold uppercase tracking-widest text-violet-400 dark:text-violet-500 whitespace-nowrap">Negocio</th>
                            <th class="px-4 py-3 text-[11px] font-bold uppercase tracking-widest text-violet-400 dark:text-violet-500 whitespace-nowrap">Empleado</th>
                            <th class="px-4 py-3 text-[11px] font-bold uppercase tracking-widest text-violet-400 dark:text-violet-500 whitespace-nowrap">Servicio</th>
                            <th class="px-4 py-3 text-[11px] font-bold uppercase tracking-widest text-violet-400 dark:text-violet-500 whitespace-nowrap">Fecha</th>
                            <th class="px-4 py-3 text-[11px] font-bold uppercase tracking-widest text-violet-400 dark:text-violet-500 whitespace-nowrap">Inicio</th>
                            <th class="px-4 py-3 text-[11px] font-bold uppercase tracking-widest text-violet-400 dark:text-violet-500 whitespace-nowrap">Fin</th>
                            <th class="px-4 py-3 text-[11px] font-bold uppercase tracking-widest text-violet-400 dark:text-violet-500 whitespace-nowrap">Estado</th>
                            <th class="px-4 py-3 text-[11px] font-bold uppercase tracking-widest text-violet-400 dark:text-violet-500 whitespace-nowrap">Notas</th>
                            <th class="px-4 py-3 text-[11px] font-bold uppercase tracking-widest text-violet-400 dark:text-violet-500 whitespace-nowrap">Acciones</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-violet-50 dark:divide-gray-700">
                        @foreach ($appointments as $appointment)
                            ...
                            <tr class="hover:bg-violet-50/50 dark:hover:bg-gray-700/50 transition-colors duration-150">
                                <td class="px-4 py-3 text-sm font-medium text-violet-950 dark:text-gray-100 whitespace-nowrap">{{ $appointment->business->name }}</td>
                                <td class="px-4 py-3 text-sm text-violet-800 dark:text-gray-300 whitespace-nowrap">{{ $appointment->employee->name }}</td>
                                <td class="px-4 py-3 text-sm text-violet-800 dark:text-gray-300 whitespace-nowrap">{{ $appointment->service_name }}</td>
                                <td class="px-4 py-3 text-sm text-violet-800 dark:text-gray-300 whitespace-nowrap">{{ $appointment->appointment_date }}</td>
                                <td class="px-4 py-3 text-sm text-violet-800 dark:text-gray-300 whitespace-nowrap">{{ $appointment->start_time }}</td>
                                <td class="px-4 py-3 text-sm text-violet-800 dark:text-gray-300 whitespace-nowrap">{{ $appointment->end_time }}</td>
                                <td class="px-4 py-3 whitespace-nowrap">
                                    <span class="inline-flex px-2 py-0.5 rounded-full text-xs font-semibold {{ $statusClasses }}">
                                        {{ $statusLabel }}
                                    </span>
                                </td>
                                <td class="px-4 py-3 text-sm text-violet-600 dark:text-violet-400 max-w-[160px] truncate">{{ $appointment->notes }}</td>
                                <td class="px-4 py-3 whitespace-nowrap">
                                    <div class="flex items-center gap-1">
                                        ...
                                        @if ($appointment->status != 'cancelled')
                                            <form action="{{ route('businesses.appointments.cancel', [$appointment->business, $appointment]) }}" method="post" class="inline">
                                                @csrf
                                                @method('PATCH')
                                                <button type="submit"
            ...
            {{-- TARJETAS: visible solo en móvil/tablet (< lg) --}}
                    ...
                    <div class="bg-white dark:bg-gray-800 border border-violet-100 dark:border-gray-600 rounded-2xl shadow-sm overflow-hidden">
                        ...
                        <div class="p-4 flex flex-col gap-3">
                            ...
                            <div class="flex items-start justify-between gap-2">
                                <div>
                                    <p class="text-base font-bold text-violet-950 dark:text-gray-100">{{ $appointment->business->name }}</p>
                                    <p class="text-sm text-violet-500 dark:text-violet-400">{{ $appointment->service_name }}</p>
                                </div>
                                <span class="inline-flex px-2 py-0.5 rounded-full text-xs font-semibold shrink-0 {{ $statusClasses }}">
                                    {{ $statusLabel }}
                                </span>
                            </div>
                            ...
                            <div class="grid grid-cols-2 gap-x-4 gap-y-2 text-sm">
                                <div>
                                    ...
                                    <p class="text-violet-800 dark:text-gray-300">{{ $appointment->employee->name }}</p>
                                </div>
                                <div>
                                ...
                                @if ($appointment->notes)
                                    <div class="col-span-2">
                                        ...
                                        <p class="text-violet-600 dark:text-violet-400 truncate">{{ $appointment->notes }}</p>
                                    </div>
                                @endif
                            </div>

                            {{-- Acciones --}}
                            <div class="flex gap-2 pt-1 border-t border-violet-50 dark:border-gray-700">
                                <a href="{{ route('businesses.appointments.show', [$appointment->business, $appointment]) }}"
                                    class="flex-1 text-center px-3 py-1.5 text-sm font-semibold bg-violet-100 dark:bg-violet-900 hover:bg-violet-200 dark:hover:bg-violet-800 text-violet-700 dark:text-violet-300 rounded-lg transition-colors duration-150">
                                    Ver
                                </a>
                                <a href="{{ route('businesses.appointments.edit', [$appointment->business, $appointment]) }}"
                                    class="flex-1 text-center px-3 py-1.5 text-sm font-semibold bg-violet-100 dark:bg-violet-900 hover:bg-violet-200 dark:hover:bg-violet-800 text-violet-700 dark:text-violet-300 rounded-lg transition-colors duration-150">
                                ...
                                @if ($appointment->status != 'cancelled')
                                    <form action="{{ route('businesses.appointments.cancel', [$appointment->business, $appointment]) }}" method="post" class="flex-1">
                                        @csrf
                                        @method('PATCH')
                                        <button type="submit"
