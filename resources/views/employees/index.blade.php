...
<body class="bg-violet-50 min-h-screen">
    ...
    <div class="max-w-4xl mx-auto px-4 py-10 flex flex-col gap-6">
        ...
        <div class="flex items-center justify-between gap-3 flex-wrap">
            <h1 class="text-2xl font-bold text-violet-950">Empleados de {{ $business->name }}</h1>
                    ...
                    <table class="w-full text-sm text-left">
                        <thead>
                            <tr class="border-b border-violet-100">
                                ...
                                <th class="px-4 py-3 text-[10px] font-bold uppercase tracking-widest text-violet-400">Teléfono</th>
                        ...
                        <tbody class="divide-y divide-violet-50">
                            @foreach ($employees as $employee)
                                <tr class="hover:bg-violet-50/50 transition-colors duration-150">
                                    <td class="px-4 py-3 text-sm font-medium text-violet-950">{{ $employee->name }}</td>
                                    <td class="px-4 py-3 text-sm text-violet-800">{{ $employee->email }}</td>
                                    <td class="px-4 py-3 text-sm text-violet-800">{{ $employee->phone }}</td>
                                    <td class="px-4 py-3">
        ...
        @else
            <div class="bg-white border border-violet-100 rounded-2xl shadow-sm px-6 py-10 text-center">
                <p class="text-sm text-violet-400">Este negocio no tiene empleados creados.</p>
            </div>
        @endif
